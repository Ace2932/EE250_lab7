...
# --- Configuration (Based on Table 1 and Lab Requirements) ---
# Note: Physical Pin 11 is BCM 17. Ensure your wiring matches the mode used.
LED_PIN    = 11   # Physical pin 11 (BOARD mode) [cite: 157, 223]
SPI_PORT   = 0
SPI_DEVICE = 0

# Sensor Channels on MCP3008 (As per your breadboard/PCB layout)
LIGHT_CHANNEL = 1
SOUND_CHANNEL = 0

# Thresholds (Requires experimentation to tune) 
LIGHT_THRESHOLD = 500 
SOUND_THRESHOLD = 600
...
def blink_led(times, interval):
    """Helper to blink LED a specific number of times [cite: 228, 230]"""
...
try:
    while True:
        # 1. Blink the LED 5 times with 500ms intervals [cite: 228, 235]
        blink_led(5, 0.5)

        # 2. Read Light Sensor for ~5 seconds at 100ms intervals [cite: 229, 235]
        print("--- Starting Light Sensor Test ---")
        for _ in range(50): # 50 * 100ms = 5 seconds
            light_val = mcp.read_adc(LIGHT_CHANNEL)
            status = "dark" if light_val < LIGHT_THRESHOLD else "bright"
            print(f"Light Raw: {light_val} - {status}")
            time.sleep(0.1)

        # 3. Blink the LED 4 times with 200ms intervals [cite: 230, 235]
        blink_led(4, 0.2)

        # 4. Read Sound Sensor for ~5 seconds at 100ms intervals [cite: 231, 235]
        print("--- Starting Sound Sensor Test ---")
        for _ in range(50): 
            sound_val = mcp.read_adc(SOUND_CHANNEL)
            print(f"Sound Raw: {sound_val}")
            
            # If tapped (above threshold), turn on LED for 100ms [cite: 232, 235]
...
# SOUND_THRESHOLD = 500   # above = tap detected        (adjust via experimentation)
